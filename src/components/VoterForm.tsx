...
export function VoterForm({ onSubmit, isLoading = false, onRandomZipCode }: VoterFormProps) {
  ...
  const handleRandomPriorities = () => {
    ...
    const randomZip = randomZipCodes[Math.floor(Math.random() * randomZipCodes.length)];
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <FormLabel className="text-base">Your Top Priorities</FormLabel>
            </div>
                ...
                {(provided) => (
                  ...
                  >
                    {Array.from({ length: 6 }).map((_, index) => (
                      <Draggable key={`priority-${index}`} draggableId={`priority-${index}`} index={index}>
                        {(provided) => (
                          <div
                            ref={provided.innerRef}
